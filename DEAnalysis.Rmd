...
```{r}
# removing genes with no p values from the raw counts
raw_filt <- raw_filt[res$Geneid,]
```
...
n_counts <- sweep(raw_filt, 2, n_fact, FUN="/")
n_counts_log2 <- log2(n_counts+1)
n_counts_log <- log10(n_counts+1)
```
...
colnames(n_counts_log2) <- paste0(colnames(n_counts_log2),"_log2")
colnames(n_counts_log) <- paste0(colnames(n_counts_log),"_log10")
colnames(n_counts) <- paste0("norm_",colnames(n_counts))
AllResults <- cbind(raw_filt, n_counts, n_counts_log2, n_counts_log)

AllResults2 <- res  %>% 
  column_to_rownames(var = "Geneid") %>% 
  dplyr::mutate(Log_baseMean = log10(baseMean),
                Mean_siC8 = apply(n_counts_log2[res$Geneid,1:4],1,mean),
                Mean_siLuc = apply(n_counts_log2[res$Geneid,5:8],1,mean),
                Rank = ifelse(padj <= 1e-30,
                              round(log2FoldChange*(-log10(1e-30))/20, 4),
                              ifelse(padj <= 1e-10,
                                     round(log2FoldChange*(-log10(1e-10))/20, 4),
                                     ifelse(padj <= 1e-5,
                                            round(log2FoldChange*(-log10(1e-5))/20, 4),
                                            ifelse(padj <= 0.1,
                                                   round(log2FoldChange*(-log10(padj))/20, 4),
                                                   round(log2FoldChange*(-log10(0.5))/20, 4))))),
                ENSEMBL = str_split_i(rownames(n_counts_log2[res$Geneid,]),"\\.",1)) %>% 
  dplyr::arrange(desc(Rank)) %>% 
  dplyr::mutate(Rank_index = seq(1,nrow(n_counts_log2[res$Geneid,]))) %>% 
  dplyr::select(ENSEMBL,Gene_name,baseMean,log2FoldChange,padj,Gene_type,
                Mean_siC8,Mean_siLuc,Log_baseMean,Rank,Rank_index)

AllResults2 %>% datatable(options = list(pageLength = 20, scrollX = T))
```

```{r ,eval=params$save}
# write.xlsx(list(Count_table = AllResults, DEA_results = AllResults2),
#            file = paste0(out_obj,"AllResults.xlsx"),
#            rowNames = TRUE)

AllResults %>% 
  rownames_to_column(var = "Geneid") %>% 
  write_xlsx(path = paste0(out_obj,"Counts_table.xlsx"))

AllResults2 %>% 
  write_xlsx(path = paste0(out_obj,"DEA_results.xlsx"))

all <- merge(AllResults2, AllResults, by = 0)
all %>% 
...
```{r}
# all %>% 
#   dplyr::filter(Gene_name %in% c("DPM1")) %>% 
#   dplyr::select(Gene_name, grep("_log10", colnames(all), value = T)) %>% 
#   column_to_rownames(var = "Gene_name") -> tmp
# 
# 
# tmp %>% 
#   pivot_longer(names_to = "Sample", values_to = "LogExp", cols = 1:8) %>% 
#   ggplot()+
